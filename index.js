...
function displayTitle(){
    var  curWeek = '5.04.2025 - 5.10.2025';
...
function displayMorningRevival(){
    ...
    const chapter = "第四周 我们的灵食、我们的灵磐石以及主筵席的内容"
...
function displayBible(){
    ...
    const old_test = "出埃及记";
    const old_test_cov = "第39~40章";
    const old_test2 = "利未记";
    const old_test_cov2 = "第1~3章";
    const old_test_link = curWeekLink;
    const new_test = "约翰福音";
    const new_test_cov = "第21章";
    const new_test2 = "使徒行传";
    const new_test_cov2 = "第1~4章";
    const new_test_link = curWeekLink;
    const content = `<h3><ul><li>旧约: ${old_test} <a href=${old_test_link}>${old_test_cov}</a> ${old_test2} <a href=${old_test_link}>${old_test_cov2}</a></li><li>新约: ${new_test} <a href=${new_test_link}>${new_test_cov}</a> ${new_test2} <a href=${new_test_link}>${new_test_cov2}</a></li></ul></h3>`;
