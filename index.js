...
function displayTitle(){
    var  curWeek = '11.16.2025 - 11.22.2025';
...
function displayBible(){
    ...
    const old_test = "士师记";
    const old_test_cov = "第18~21章"
    const old_test1 = "路得记";
    const old_test_cov1 = "第1章"
    const old_test_link = curWeekLink;
    const new_test = "彼得后书";
    const new_test_cov = "第2~3章";
    const new_test1 = "约翰一书";
    const new_test_cov1 = "第1~3章";
    const new_test_link = curWeekLink;
    const content = `<h3><ul><li>旧约: ${old_test} <a href=${old_test_link}>${old_test_cov}</a> ${old_test1} <a href=${old_test_link}>${old_test_cov1}</a></li><li>新约: ${new_test} <a href=${new_test_link}>${new_test_cov}</a>, ${new_test1} <a href=${new_test_link}>${new_test_cov1}</a></li></ul></h3>`;
...
function displayLifeStudy(){
    ...
    const book = "雅歌";
    const coverage = "雅歌五章2节～六章13节展覧(追求材料：雅歌生命读经 [第六，七篇]，雅歌结晶读经 [第十至十二篇]，歌中之歌 [第四段])";
    const link = curWeekLink;
    const date = "11.20.2025";
